...
factor_map_dic = dict(zip(df_factor["Source Name"], df_factor["Factor Value[Group]"]))
...
#### get the control samples columns from the original dataframe: df_height
control_df = df_height[
    [
        original_name
        for modify_name, original_name in zip(column_name_list, df_height.columns)
        if factor_map_dic[modify_name] == "Control"
    ]
]

#### get the POP samples columns from the original dataframe: df_height
pop_df = df_height[
    [
        original_name
        for modify_name, original_name in zip(column_name_list, df_height.columns)
        if factor_map_dic[modify_name] == "POP"
    ]
]

#### do t-test for each metabolite
t_stat_list, pvalue_list = ttest_ind(control_df.values, pop_df.values, axis=1)
...
#### output the files
df_output.to_csv(
    "/Users/yliao13/PycharmProjects/phd_class/Lab_work/data_files/MTBLS1033/Urine/POS/new_analysis/mummichog/mummichog_input_mtbls1033_urine_positive.csv",
    sep="\t",
)

df_output_statistic.to_csv(
    "/Users/yliao13/PycharmProjects/phd_class/Lab_work/data_files/MTBLS1033/Urine/POS/new_analysis/mummichog/mummichog_statistics_mtbls1033_urine_positive.csv",
    sep="\t",
)

...
df_origin = df_original.loc[:, '01_Brain': '03_Plasma01']
...
for i in df_origin.columns:
    if 'Brain' in str(i):
        ...
        target_color.append(0)
    if 'Heart' in str(i) or 'Liver' in str(i):
        target[target_index] = [0, 1, 0]   # Cow
        target_color.append(1)
    if 'Muscle' in str(i) or 'Plasma01' in str(i):
...
df_out = pd.DataFrame(columns=df_original.columns)
df_out['VIP'] = None

n_index = 0
for i in vip_index:
    if vip_value[i] > 1.8:
        df_out = df_out.append(df_original.iloc[i])
